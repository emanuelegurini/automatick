#!/usr/bin/env python3
"""Knowledge A2A Runtime — uses Gateway MCP for cross-account AWS access.
...
def _get_a2a_server():
    ...
    if _a2a_server is None:
        ...
        agent = create_context_agent(
            ...
            description="Searches AWS knowledge base for troubleshooting and best practices",
            system_prompt=KNOWLEDGE_PROMPT,
            mcp_client=mcp_client,
        )
        _a2a_server = create_a2a_server(agent, _runtime_url)
        logger.info("Knowledge A2A server initialized")
    return _a2a_server
...
def create_app() -> FastAPI:
    app = FastAPI(title="Knowledge A2A Runtime", lifespan=lifespan)
    app.add_api_route("/ping", ping, methods=["GET"])
    app.mount("/", _LazyA2AApp())
    logger.info("Knowledge A2A Runtime ready (Gateway connection deferred to first request)")
    return app
