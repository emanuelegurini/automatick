"""Robust Date Parsing Module — natural language period resolution for cost queries.

Architecture: four-tier hybrid pipeline with confidence thresholds at each stage.

  Tier 1 — LLM structured output (confidence >= 0.8): delegates to a Strands Agent
            that returns a JSON object with exact ISO dates. Only available when an
            llm_agent is injected (not in supervisor_tools.py, which passes None to
            avoid adding a second model call on every cost query).

  Tier 2 — Specialized libraries (confidence >= 0.7): parsedatetime handles complex
            natural language ("next Tuesday", "in two weeks"); dateutil relativedelta
            handles calendar-accurate month/year arithmetic (avoids 30-day month errors).

  Tier 3 — Enhanced regex (confidence >= 0.6): fast pattern matching for the most
            common MSP business phrases ("last 3 months", "YTD", "current bill", etc.)
            without any external calls. This is the primary path in production because
            supervisor_tools.py instantiates the parser with llm_agent=None.

  Tier 4 — Fallback (confidence = 0.3): returns 30 days as a safe default when all
            other tiers fail. Always succeeds; never raises.

All tiers return DateRange (ISO 8601 strings) so callers need no date arithmetic.
"""
...
class DateRange:
    ...
    Attributes:
        start_date: Inclusive range start in ISO 8601 format (YYYY-MM-DD).
        end_date: Inclusive range end in ISO 8601 format (YYYY-MM-DD).
        period_days: Approximate number of days in the range (used by callers
            that need a single integer for Cost Explorer granularity decisions).
        confidence: Parser confidence in [0.0, 1.0]. Values below 0.6 indicate
            the fallback path was used and the dates may not match user intent.
        interpretation: Human-readable explanation of what the parser understood;
            useful for debugging mismatches between user language and resolved dates.
        method_used: One of LLM_STRUCTURED | PARSEDATETIME | DATEUTIL |
            ENHANCED_REGEX | FALLBACK — identifies which tier produced the result.
    """
...
class RobustDateParser:
    ...
    def _parse_with_llm_structured(self, query: str) -> Optional[DateRange]:
        """Use an LLM agent to parse the query and return a structured DateRange.

        Injects today's date, current day-of-month, and YTD days into the prompt
        so the model can perform exact arithmetic without hallucinating dates.

        Args:
            query: Natural language string containing a time period reference.

        Returns:
            DateRange if the LLM returns valid JSON with all required fields and
            confidence >= threshold; None if no llm_agent is configured, the JSON
            is malformed, or date strings fail ISO 8601 validation.
        """
    ...
    def _parse_with_libraries(self, query: str) -> Optional[DateRange]:
        """Attempt date parsing using parsedatetime and dateutil libraries.

        Two sub-strategies are tried in order:
          2A. parsedatetime.Calendar.parse() — handles free-form relative expressions
              like "two weeks ago" or "last Tuesday". Returns a time_struct; a non-zero
              parse_status indicates a successful parse.
          2B. dateutil relativedelta — calendar-accurate month/year arithmetic. Regex
              first extracts the numeric quantity and unit, then relativedelta computes
              the exact start date without the 30-day-month approximation error that
              plain timedelta would introduce.

        Args:
            query: Natural language string containing a time period reference.

        Returns:
            DateRange on success; None if neither sub-strategy matches or if the
            computed period_days falls outside the sanity bounds [1, 1095].
        """
        try:
    ...
    def _parse_with_enhanced_regex(self, query: str) -> Optional[DateRange]:
        """Match common MSP business time expressions using compiled regex patterns.

        This is the primary production path because the parser is instantiated with
        llm_agent=None in supervisor_tools.py (to avoid a second model round-trip on
        every cost query). Patterns are ordered from most to least specific; the first
        match wins.

        The calculator lambdas capture the match object and return an integer number
        of days, keeping period arithmetic co-located with each pattern for readability.

        Args:
            query: Natural language string containing a time period reference.

        Returns:
            DateRange on the first pattern match; None if no pattern matches or the
            computed period_days falls outside the sanity bounds [1, 1095].
        """
        try:
            query_lower = query.lower()
    ...
    def _get_quarter_days(self) -> int:
        """Calculate the number of days elapsed so far in the current calendar quarter.

        Returns:
            int: Days from the start of the current quarter (Jan/Apr/Jul/Oct 1) through
            today, inclusive. Minimum value is 1 (called on the first day of a quarter).
        """
    ...
    def _fallback_parse(self, query: str) -> DateRange:
        """Return a safe 30-day default range when all parsing tiers fail.

        This method never raises. The low confidence score (0.3) signals to callers
        that the dates are a best-effort guess rather than an authoritative parse.

        Args:
            query: Original query string (unused in computation; kept for symmetry
                with other tier methods and potential future logging).

        Returns:
            DateRange covering the last 30 days with confidence=0.3 and
    ...
    def get_test_cases(self) -> Dict[str, DateRange]:
        """Parse a fixed set of representative queries and return the results.

        Used by validate_date_parser() and in unit tests to exercise all four tiers.
        Results vary by current date because the parser anchors all relative expressions
        to datetime.now() captured at __init__ time.

        Returns:
            Dict mapping each test query string to its DateRange result.
        """
        test_queries = [
            "last 4 months",
            "last 6 months", 
...
def create_robust_date_parser(llm_agent=None) -> RobustDateParser:
    """Factory function to create a configured RobustDateParser.

    Prefer this over direct instantiation so call sites do not need to import
    RobustDateParser explicitly, and so future constructor changes only need to
    be updated here.

    Args:
        llm_agent: Optional Strands Agent instance for Tier-1 LLM parsing.
            Pass None (default) to skip LLM parsing and use library/regex tiers only.

    Returns:
        RobustDateParser: Ready-to-use parser instance anchored to the current date.
    """
    return RobustDateParser(llm_agent=llm_agent)


# Test validation function
def validate_date_parser() -> Dict[str, DateRange]:
    """Run the built-in test suite and log results for each query.

    Instantiates a parser without an LLM agent (regex/library tiers only) and
    calls get_test_cases(). Intended for quick smoke-testing during deployment
    or local development; run this file directly with `python robust_date_parser.py`.
